...
def generate_sample_data(
    ...
    for symbol in symbols:
        ...
        seasonality = 0.05 * np.sin(2 * np.pi * np.arange(n_days) / 252)  # Annual seasonality
        ...
        for i, date in enumerate(dates):
            ...
            volume_multiplier = 1 + daily_volatility * 10  # Higher volume on volatile days
    ...
    # Save to CSV
    filename = f"sample_data_{start_date.strftime('%Y%m%d')}_{end_date.strftime('%Y%m%d')}.csv"
    filepath = output_path / filename
    
    df.to_csv(filepath, index=False)
    
    logger.info(
        f"Generated {len(df)} rows of sample data saved to {filepath}"
    )
