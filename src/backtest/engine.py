...
from src.backtest.events import BarEvent, Event, FillEvent, OrderEvent, SignalEvent
...
class BacktestEngine:
    ...
    def __init__(
        ...
        self.execution_handler = SimulatedExecutionHandler(
            commission_per_share=commission_per_share, slippage_bps=slippage_bps
        )
    ...
    def _handle_bar_event(self, event: BarEvent) -> None:
        ...
        try:
            ...
            orders = self._generate_orders_from_targets(
                target_positions, event.timestamp
            )
            for order in orders:
    ...
    def get_results(self) -> Dict:
        ...
        trades_df = (
            pd.DataFrame(self.trade_history) if self.trade_history else pd.DataFrame()
        )
        signals_df = (
            pd.DataFrame(self.signal_history) if self.signal_history else pd.DataFrame()
        )
