...
class MeanReversionStrategy(BaseStrategy):
    ...
    def find_cointegrated_pairs(
        ...
        if (
            self.last_pair_selection is not None
            and (timestamp - self.last_pair_selection).days < 7
        ):
        ...
        for i in range(len(symbols)):
            for j in range(i + 1, len(symbols)):
                ...
                try:
                    ...
                    self.logger.debug(
                        f"Cointegration test failed for {symbol1}-{symbol2}: {e}"
                    )
                    continue
    ...
    def calculate_pair_signals(
        ...
        for symbol1, symbol2 in self.cointegrated_pairs:
            try:
                if (
                    symbol1 not in price_pivot.columns
                    or symbol2 not in price_pivot.columns
                ):
                    continue
                ...
                if abs(zscore) > self.zscore_threshold:
                    signal_strength = (
                        min(abs(zscore) / self.zscore_threshold, 2.0) / 2.0
                    )
                ...
                self.logger.debug(
                    f"Error calculating pair signal for {symbol1}-{symbol2}: {e}"
                )
                continue
        ...
        if pair_signals:
            ...
            for pair_key, pair_signal_list in pair_signals.items():
                ...
                for signal in pair_signal_list:
                    if (
                        signal.symbol in latest_prices
                        and latest_prices[signal.symbol] > 0
                    ):
